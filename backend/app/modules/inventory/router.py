...
from app.modules.inventory.schemas import ProductCreate, ProductUpdate, ProductOut, LotCreate
...
def update_product(product_id: str, data: ProductUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return inventory_service.update_product(db, product_id, data.model_dump(exclude_unset=True))
