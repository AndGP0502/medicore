...
2. Backend (requiere **Python 3.12**; 3.14 aún no tiene wheels de algunas deps): crear venv en `backend/`, `pip install -r requirements.txt`, definir `.env` (ver `deploy/medicore.env.example`) y correr `uvicorn app.main:app`. Crear admin: `ADMIN_EMAIL=... ADMIN_PASSWORD=... python crear_admin.py`.
