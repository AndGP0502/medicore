    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1001, background: 'rgba(10,61,107,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', fontFamily: "'Nunito','Segoe UI',sans-serif" }}
      ...
      <div style={{ background: 'white', borderRadius: '18px', width: '100%', maxWidth: '400px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.78rem', fontWeight: '700', color: '#374151', textTransform: 'uppercase' }}>Monto *</label>
            <input type="number" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} style={INPUT} step="0.01" min="0" />
          </div>
          <div>
            <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.78rem', fontWeight: '700', color: '#374151', textTransform: 'uppercase' }}>Metodo *</label>
      ...
      <div style={{ background: 'white', borderRadius: '18px', width: '100%', maxWidth: '500px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(135deg, #0a3d6b, #0d5fa3)', padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button onClick={() => window.print()} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', borderRadius: '8px', color: 'white', cursor: 'pointer', padding: '6px 12px', fontSize: '0.8rem', fontWeight: '700', fontFamily: 'inherit' }}>Imprimir</button>
            <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: '8px', color: 'white', cursor: 'pointer', padding: '6px 10px' }}>x</button>
          </div>
        </div>
        <div style={{ padding: '1.5rem' }}>
          <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
            ...
            {invoice.clave_acceso && <p style={{ margin: '0.25rem 0 0', fontSize: '0.7rem', color: '#9ca3af', wordBreak: 'break-all' }}>Clave: {invoice.clave_acceso}</p>}
          </div>
          ...
          <div style={{ borderTop: '2px solid #e8edf2', paddingTop: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.3rem', fontSize: '0.875rem', color: '#4b5563' }}><span>Subtotal:</span><span>${parseFloat(invoice.subtotal).toFixed(2)}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.3rem', fontSize: '0.875rem', color: '#4b5563' }}><span>IVA:</span><span>${parseFloat(invoice.tax).toFixed(2)}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.1rem', fontWeight: '800', color: '#0a3d6b' }}><span>TOTAL:</span><span>${parseFloat(invoice.total).toFixed(2)}</span></div>
          </div>
        </div>
      </div>
    </div>
...
function InvoiceModal({ onClose }) {
  ...
  const [items, setItems] = useState([{ description: '', quantity: '1', unit_price: '', iva_porcentaje: '0' }])
  ...
  const mutation = useMutation({
    ...
    onError: (err) => { const d = err?.response?.data?.detail; toast.error(Array.isArray(d) ? d[0]?.msg : (typeof d === 'string' ? d : 'Error')) },
  })
  ...
  function handleSubmit(e) {
    e.preventDefault()
    const payload = { patient_id: form.patient_id, notes: form.notes || undefined, items: items.filter(i => i.description && i.unit_price).map(i => ({ description: i.description, quantity: i.quantity, unit_price: i.unit_price, iva_porcentaje: i.iva_porcentaje })) }
  ...
  const ivaTotal = items.reduce((acc, i) => acc + (parseFloat(i.quantity || 0) * parseFloat(i.unit_price || 0) * (parseFloat(i.iva_porcentaje || 0) / 100)), 0)
        ...
        <form onSubmit={handleSubmit} style={{ overflow: 'auto', flex: 1 }}>
          <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div>
              ...
              {items.map((item, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '3fr 0.8fr 1fr 1fr auto', gap: '0.5rem', marginBottom: '0.5rem' }}>
                  ...
                  <select value={item.iva_porcentaje} onChange={e => setItem(i, 'iva_porcentaje', e.target.value)} style={INPUT} title="Tarifa de IVA">
                    <option value="0">IVA 0%</option>
                    <option value="15">IVA 15%</option>
                  </select>
                  {items.length > 1 && <button type="button" onClick={() => setItems(prev => prev.filter((_, idx) => idx !== i))} style={{ background: '#fee2e2', border: 'none', borderRadius: '8px', color: '#dc2626', cursor: 'pointer', padding: '0 0.75rem', fontWeight: '700' }}>x</button>}
                </div>
              ))}
              <button type="button" onClick={() => setItems(prev => [...prev, { description: '', quantity: '1', unit_price: '', iva_porcentaje: '0' }])} style={{ padding: '0.5rem 1rem', border: '1.5px dashed #e2e8f0', borderRadius: '8px', background: 'none', cursor: 'pointer', fontSize: '0.875rem', color: '#6b7c93', fontFamily: 'inherit' }}>+ Agregar item</button>
            </div>
            <div style={{ background: '#f8fafc', borderRadius: '10px', padding: '1rem', border: '1px solid #e8edf2' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', color: '#4b5563', marginBottom: '0.3rem' }}><span>IVA:</span><span>${ivaTotal.toFixed(2)}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem', fontWeight: '800', color: '#0a3d6b' }}><span>Total:</span><span>${(subtotal + ivaTotal).toFixed(2)}</span></div>
            </div>
          </div>
          <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid #f0f4f8', display: 'flex', justifyContent: 'space-between', background: '#fafafa' }}>
...
export default function BillingPage() {
  ...
  async function handleEmitir(inv) {
    setEmitiendo(inv.id)
    try {
      const toastId = toast.loading('Enviando al SRI... esto puede tomar hasta 60 segundos')
      const res = await emitirFactura(inv.id)
      toast.dismiss(toastId)
      if (res.data.ok) {
        toast.success(`Factura autorizada. Autorizacion: ${res.data.numero_autorizacion}`)
        queryClient.invalidateQueries({ queryKey: ['invoices'] })
      } else {
        toast.error(`Error SRI: ${res.data.error || res.data.mensajes || 'Sin respuesta'}`)
      }
    } catch (err) {
      toast.dismiss()
      const d = err?.response?.data?.detail
      toast.error(typeof d === 'string' ? d : 'Error al emitir')
    } finally {
      setEmitiendo(null)
    }
  }
  ...
  const sriConfigurado = configSRI?.configured && configSRI?.tiene_certificado
    ...
    <div style={{ fontFamily: "'Nunito','Segoe UI',sans-serif" }}>
      ...
      <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          ...
          <p style={{ margin: 0, color: '#6b7c93', fontSize: '0.875rem' }}>Gestion de facturas electronicas</p>
        </div>
      ...
      {!sriConfigurado && (
        <div style={{ background: '#fef9c3', border: '1px solid #fde68a', borderRadius: '12px', padding: '0.875rem 1.25rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span style={{ fontSize: '1.2rem' }}>!</span>
          <div style={{ flex: 1 }}>
            <p style={{ margin: 0, fontWeight: '700', fontSize: '0.875rem', color: '#a16207' }}>SRI no configurado</p>
            <p style={{ margin: 0, fontSize: '0.8rem', color: '#a16207' }}>Configura los datos del SRI y sube el certificado .p12 para emitir facturas electronicas.</p>
          </div>
          <button onClick={() => setShowConfigSRI(true)} style={{ padding: '0.5rem 1rem', background: '#a16207', color: 'white', border: 'none', borderRadius: '8px', fontSize: '0.8rem', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
            Configurar ahora
          </button>
        </div>
      )}
      ...
      <div style={{ background: 'white', borderRadius: '14px', border: '1px solid #e8edf2', overflow: 'hidden' }}>
        {isLoading ? <div style={{ padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>Cargando...</div> : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                {['Numero', 'Total', 'Estado', 'SRI', 'Acciones'].map(h => (
            ...
            <tbody>
              {items.map((inv, i) => {
                ...
                const yaEmitida = inv.status === 'paid'
                return (
                  <tr key={inv.id} style={{ borderBottom: i < items.length-1 ? '1px solid #f0f4f8' : 'none' }}>
                    ...
                    <td style={{ padding: '1rem 1.25rem' }}>
                      {yaEmitida
                        ? <span style={{ padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: '700', background: '#dcfce7', color: '#16a34a' }}>Autorizada</span>
                        : <span style={{ padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: '700', background: '#f3f4f6', color: '#6b7c93' }}>Pendiente</span>
                      }
                    </td>
                    <td style={{ padding: '1rem 1.25rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      <button onClick={() => setPrintInvoice(inv)} style={{ padding: '0.35rem 0.75rem', background: '#eff6ff', color: '#0d5fa3', border: '1px solid #bfdbfe', borderRadius: '6px', fontSize: '0.75rem', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>Ver</button>
                      {!yaEmitida && (
                        <button onClick={() => setPaymentInvoice(inv)} style={{ padding: '0.35rem 0.75rem', background: '#dcfce7', color: '#16a34a', border: '1px solid #bbf7d0', borderRadius: '6px', fontSize: '0.75rem', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>Cobrar</button>
                      )}
                      {!yaEmitida && sriConfigurado && (
                        <button onClick={() => handleEmitir(inv)} disabled={emitiendo === inv.id}
                          style={{ padding: '0.35rem 0.75rem', background: emitiendo === inv.id ? '#e2e8f0' : '#fef9c3', color: emitiendo === inv.id ? '#9ca3af' : '#a16207', border: '1px solid #fde68a', borderRadius: '6px', fontSize: '0.75rem', fontWeight: '700', cursor: emitiendo === inv.id ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
                          {emitiendo === inv.id ? 'Emitiendo...' : 'Emitir SRI'}
                        </button>
                      )}
                      <button onClick={() => { if (window.confirm('Eliminar esta factura?')) deleteMutation.mutate(inv.id) }} style={{ padding: '0.35rem 0.75rem', background: '#fee2e2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '6px', fontSize: '0.75rem', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>Eliminar</button>
                    </td>
                  </tr>
                )
              })}
              {items.length === 0 && (
                <tr><td colSpan={5} style={{ padding: '3rem', textAlign: 'center', color: '#9ca3af' }}>No hay facturas registradas</td></tr>
              )}
            </tbody>
          </table>
        )}
