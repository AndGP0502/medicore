...
function Message({ msg }) {
  return (
    <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
      ...
      {msg.role === 'user' && (
        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#e8edf2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0 }}>?????</div>
      )}
    </div>
  )
}
    ...
    <div style={{ fontFamily: "'Nunito','Segoe UI',sans-serif", height: 'calc(100vh - 3rem)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={handleClear}
          style={{ padding: '0.6rem 1.25rem', background: '#fee2e2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '10px', fontSize: '0.875rem', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>
          Nuevo paciente
        </button>
      </div>
          ...
          <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid #f0f4f8' }}>
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end' }}>
              <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
                placeholder={currentAction?.placeholder} rows={2}
