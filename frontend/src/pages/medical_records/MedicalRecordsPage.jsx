  ...
  return (
    <div style={{ fontFamily: "'Nunito','Segoe UI',sans-serif" }}>
      {showModal && selectedPatient && (
        <MedicalRecordModal patient={selectedPatient} onClose={() => setShowModal(false)} />
      )}
      {selectedRecord && (
        <MedicalRecordDetail record={selectedRecord} onClose={() => setSelectedRecord(null)} />
      )}
          ...
          <div style={{ maxHeight: '600px', overflowY: 'auto' }}>
            {patients.map(p => (
              <div key={p.id}
                ...
                style={{
                  padding: '0.875rem 1rem', cursor: 'pointer', borderBottom: '1px solid #f0f4f8',
                  background: selectedPatient?.id === p.id ? '#eff6ff' : 'transparent',
                  borderLeft: selectedPatient?.id === p.id ? '3px solid #0d5fa3' : '3px solid transparent',
                }}>
